using FreeCource.Services.Order.Application.Commands;
...
namespace FreeCource.Services.Order.Application.Handlers
{
    public class CreateOrderCommandHandler : IRequestHandler<CreateOrderCommand, Response<CreatedOrderDto>>
    {
        ...
        public async Task<Response<CreatedOrderDto>> Handle(CreateOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            var newOrder = new FreeCourse.Services.Order.Domain.OrderAggregate.Order(request.BuyerId, newAddress);
            request.OrderItems.ForEach(x =>
            {
                newOrder.AddOrderOtem(x.ProductId, x.ProductName, x.Price, x.PictureUrl);
            });
